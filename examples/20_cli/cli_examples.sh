...
# --timeout 5: 设置测速超时（秒）
# --format table: 以表格形式输出（默认 JSON）
# tdxman ping [--timeout 5] [--format json|table|csv] [--output PATH]
# 输出:
# [
...
# tdxman kline SZ 000001 --count 5 --format table
# 输出:
...
# tdxman kline SH 600519 --adjust QFQ --period DAILY --format table
# 输出:
#         datetime      open      high       low     close    volume        amount
...
# tdxman quote-list A --count 10 --format table
# 输出:
...
# tdxman transaction SZ 000001 --count 20 --format table
# 输出:
...
# tdxman board-list --type GN --count 10 --format table
# 输出:
...
# 返回列: market, code, name, price, change_pct, volume, amount
# tdxman board-members 881001 --count 10 --format table
# 输出:
#  market   code     name   price  change_pct    vol      amount
...
# tdxman unusual SZ --count 20 --format table
# 输出:
...
# tdxman ex kline HK_MAIN_BOARD 00700 --count 10 --format table
# 输出:
#         datetime   open   high    low  close    volume      amount
...
# tdxman ex quote-list HK_MAIN_BOARD --count 10 --format table
# 输出:
#        code           name  market
#      00001         长和      31
#      00002         中电控股    31
