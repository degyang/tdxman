...
class TickChartsCmd(BaseCommand[MacMultiTickChart]):
    ...
    def parse_response(self, body: bytes) -> MacMultiTickChart:
        ...
        # `total` is the combined number of ticks for every returned day.  On
        # an in-progress trading day, the first day has fewer than page_size
        # ticks; subsequent historical days have page_size ticks.  Reading
        # page_size records for every day runs into the trailing metadata.
        first_day_ticks = max(0, total - (count - 1) * page_size)
        tick_index = 0
        for d in range(count):
            ticks: list[MacTick] = []
            ticks_in_day = first_day_ticks if d == 0 else page_size
            ticks_in_day = min(ticks_in_day, total - tick_index)
            for _ in range(ticks_in_day):
                record_index = tick_index
                offset = 71 + record_index * 14
                (minutes, price, avg, vol, _tick_reserved) = unpack_from(
                    "<HffHH", body, offset, f"tick_charts tick[{d}][{record_index}]"
                )
                tick_index += 1
                ticks.append(
                    MacTick(
