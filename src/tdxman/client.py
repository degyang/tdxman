...
_CACHE_DIR = Path.home() / ".tdxman" / "cache"
...
class TdxClient:
    ...
    def get_history_fund_flow(
        ...
        try:
            ...
            dates = [_date_from_bar(bar) for bar in bars]
        except TdxDecodeError:
            # Some current standard-protocol servers return an incomplete day-bar
            # response. MAC daily bars provide the same trading dates, while the
            # historical transaction requests below remain on the standard API.
            from .mac.client import MacClient
            from .mac.enums import Adjust, Period

            with MacClient.from_best_host(timeout=self._timeout) as mac_client:
                mac_bars = mac_client.get_stock_kline(
                    market, code, Period.DAILY, start, count, adjust=Adjust.NONE
                )
            dates = [int(pd.Timestamp(value).strftime("%Y%m%d")) for value in mac_bars["datetime"]]
