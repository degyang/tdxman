...
优先级：环境变量 > ~/.tdxman/config.json > 源码内嵌默认值。
...
环境变量覆盖::
    ...
    TDXMAN_PORT        -- 端口
    TDXMAN_TIMEOUT     -- 超时秒数
    TDXMAN_KNOWN_HOSTS -- 逗号分隔的候选主机列表
    TDXMAN_CONFIG_DIR  -- 配置文件目录（默认 ~/.tdxman）
"""
...
_CONFIG_DIR = Path(os.environ.get("TDXMAN_CONFIG_DIR", str(Path.home() / ".tdxman")))
...
def get_port() -> int:
    """返回默认端口。"""
    env = os.environ.get("TDXMAN_PORT")
    if env:
        return int(env)
    cfg = _load()
...
def get_timeout() -> float:
    """返回默认超时秒数。"""
    env = os.environ.get("TDXMAN_TIMEOUT")
    if env:
        return float(env)
    cfg = _load()
